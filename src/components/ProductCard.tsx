...
import { ShoppingBag, Plus, AlertCircle, Trash2 } from "lucide-react";
...
export default function ProductCard({
  ...
  const handleAdd = () => {
    const success = addItem({ productId: id, name, price, image }, stock);
    if (!success) {
    ...
    <Link href={`/products/${id}`} className="block">
      <article className="group relative flex flex-col rounded-2xl bg-card border border-border overflow-hidden transition-all duration-500 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1">
        ...
        <div className="relative aspect-square overflow-hidden bg-surface">
          ...
          {/* Error notification */}
          {error && (
            <div className="absolute bottom-3 start-3 end-3 flex items-center gap-2 rounded-lg bg-red-500/20 backdrop-blur-sm px-3 py-2 text-[11px] font-medium text-red-300">
          ...
          {isInCart ? (
            <button
              ...
              className="hidden sm:flex absolute bottom-3 end-3 items-center gap-1.5 rounded-full bg-danger/90 backdrop-blur-sm px-3.5 py-2 text-xs font-medium text-white shadow-lg shadow-danger/20 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 hover:bg-danger"
            >
      ...
      <div className="flex flex-1 flex-col justify-between p-3 sm:p-5">
        ...
        {isInCart ? (
          <button
            onClick={handleRemove}
            className="sm:hidden flex w-full items-center justify-center gap-1.5 rounded-full bg-danger/90 px-3 py-1.5 mt-2 text-[11px] font-medium text-white"
          >
